#!/usr/bin/env python3

'''
Dumps quarantined files from Windows Defender

'--quarantine-path' provide full path to Defender quarantine folder, this is good for devices obtained from kape. 
Example "Device\C%3A\ProgramData\Windows Defender\Quarantine"
'rootdir' Root directory where Defender is installed (example C:\)'
'--dump' Dump all entries into tar archive (quarantine.tar)

Inspired by https://github.com/knez/defender-dump/tree/master
'''

import io
import struct
import argparse
import datetime
import pathlib
import tarfile

from collections import namedtuple
file_record = namedtuple("file_record", "path hash detection filetime")

def mse_ksa():
    # hardcoded key obtained from mpengine.dll
    key = [
        
    ]
    sbox = list(range(256))
    j = 0
    for i in range(256):
        j = (j + sbox[i] + key[i]) % 256
        tmp = sbox[i]
        sbox[i] = sbox[j]
        sbox[j] = tmp
    return sbox

def rc4_decrypt(data):
    sbox = mse_ksa()
    out = bytearray(len(data))
    i = 0
    j = 0
    for k in range(len(data)):
        i = (i + 1) % 256
        j = (j + sbox[i]) % 256
        tmp = sbox[i]
        sbox[i] = sbox[j]
        sbox[j] = tmp
        val = sbox[(sbox[i] + sbox[j]) % 256]
        out[k] = val ^ data[k]

    return out

def unpack_malware(f):

    decrypted = rc4_decrypt(f.read())
    sd_len = struct.unpack_from('<I', decrypted, 0x8)[0]
    header_len = 0x28 + sd_len
    malfile_len = struct.unpack_from('<Q', decrypted, sd_len + 0x1C)[0]
    malfile = decrypted[header_len:header_len + malfile_len]

    return (malfile, malfile_len)

def dump_entries(basedir, entries):

    tar = tarfile.open('quarantine.tar', 'w')

    for file_rec in entries:
        quarfile = basedir / 'ResourceData' / file_rec.hash[:2] / file_rec.hash

        if not quarfile.exists():
            continue

        with open(quarfile, 'rb') as f:

            print(f'Exporting {file_rec.path.name}')
            malfile, malfile_len = unpack_malware(f)

            tarinfo = tarfile.TarInfo(file_rec.path.name)
            tarinfo.size = malfile_len
            tar.addfile(tarinfo, io.BytesIO(malfile))

    tar.close()

    print("File 'quarantine.tar' successfully created")

def get_entry(data):

    # extract path as a null-terminated UTF-16 string
    pos = data.find(b'\x00\x00\x00') + 1
    path_str = data[:pos].decode('utf-16le')

    # normalize the path
    if path_str[2:4] == '?\\':
        path_str = path_str[4:]

    path = pathlib.PureWindowsPath(path_str)

    pos += 4  # skip number of entries field
    type_len = data[pos:].find(b'\x00')
    type = data[pos:pos + type_len].decode()  # get entry Type (UTF-8)
    pos += type_len + 1
    pos += (4 - pos) % 4  # skip padding bytes
    pos += 4  # skip additional metadata
    hash = data[pos:pos + 20].hex().upper()

    return (path, hash, type)

def parse_entries(basedir):

    results = []
    for guid in basedir.glob('Entries/{*}'):
        with open(guid, 'rb') as f:
            header = rc4_decrypt(f.read(0x3c))
            data1_len, data2_len = struct.unpack_from('<II', header, 0x28)

            data1 = rc4_decrypt(f.read(data1_len))
            filetime, = struct.unpack('<Q', data1[0x20:0x28])
            filetime = datetime.datetime(1970, 1, 1) + datetime.timedelta(microseconds=filetime // 10 - 11644473600000000)
            detection = data1[0x34:].decode('utf8')

            data2 = rc4_decrypt(f.read(data2_len))
            cnt = struct.unpack_from('<I', data2)[0]
            offsets = struct.unpack_from('<' + str(cnt) + 'I', data2, 0x4)

            for o in offsets:
                path, hash, type = get_entry(data2[o:])
                if type == 'file':
                    results.append(file_record(path, hash, detection, filetime))

    return results

def get_entries(quarantine_dir):
    # Placeholder for logic to populate entries from the quarantine folder.
    # Replace this with the actual logic that reads the quarantine files.
    # Example format: [(id, timestamp, detection_name, ...)]
    return parse_entries(quarantine_dir)

def main(args):
    # Determine the quarantine directory
    if args.quarantine_path:
        quarantine_dir = args.quarantine_path
    elif args.rootdir:
        quarantine_dir = args.rootdir / r'ProgramData/Microsoft/Windows Defender/Quarantine'
    else:
        print("Error: Either --quarantine-path or rootdir must be specified.")
        return

    quarantine_dir = pathlib.Path(quarantine_dir)  # Ensure Path object

    if not quarantine_dir.exists():
        print(f"Error: Quarantine directory does not exist: {quarantine_dir}")
        return

    print(f"Using quarantine directory: {quarantine_dir}")

    # Parse entries
    entries = parse_entries(quarantine_dir)
    if not entries:
        print("No quarantine entries found.")
        return

    print(f"Found {len(entries)} quarantine entries.")

    # Dump or list entries
    if args.dump:
        print("Dumping entries into tar archive...")
        dump_entries(quarantine_dir, entries)
    else:
        detection_max_len = max(len(entry.detection) for entry in entries)
        for entry in entries:
            print(f"{entry.filetime} {entry.detection:<{detection_max_len}} {entry.path}")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Dump quarantined files from Windows Defender'
    )
    parser.add_argument(
        'rootdir', type=pathlib.Path, nargs='?', default=None,
        help=r'Root directory where Defender is installed (example C:\)'
    )
    parser.add_argument(
        '-q', '--quarantine-path', type=pathlib.Path,
        help='Full path to the quarantine folder (overrides rootdir)'
    )
    parser.add_argument(
        '-d', '--dump', action='store_true',
        help='Dump all entries into tar archive (quarantine.tar)'
    )
    args = parser.parse_args()
    main(args)